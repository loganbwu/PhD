...
griffin_data = tibble(t = seq(0, years, length.out=500),
       `Delayed\nε=0, κ=1, ψ−=π/4` = make_m(0, 1, years/2)(t),
       `Sharp trough\nε=0, κ=1/4` = make_m(0, 1/4, years/4)(t),
       `Sharp peak\nε=0, κ=4` = make_m(0, 4, years/4)(t),
       `Nonseasonal\nε=1` = make_m(1, 1, years/4)(t),
       `Shallow\nε=1/2, κ=1` = make_m(1/2, 1, years/4)(t),
       `Sinusoidal\nε=0, κ=1` = make_m(0, 1, years/4)(t)) %>%
  pivot_longer(-t) %>%
...
ggplot(griffin_data, aes(x = t/years*12, y = value, color = name)) +
  ...
  labs(title = "Transformations of the Griffin seasonality function",
       x = "Month",
...
plot_data_map = map_sites %>%
  select(region = shapeName, month, map = value) %>%
  group_by(region) %>%
  mutate(map = map / mean(map))
plot_data_m = expand_grid(
  month = seq(1, 12, length.out=100),
  region = c("Huangchuan", "Wanning"),
  m = NA_real_
)
for (i in seq_len(nrow(plot_data_m))) {
  day = plot_data_m$month[i] * (365.25/12) + 15
  region = plot_data_m$region[i]
  plot_data_m$m[i] = fit_functions[[region]](day)
}
for (i in seq_len(nrow(plot_data))) {
  day = map_sites$month[i] * (365.25/12) + 15
  region = map_sites$shapeName[i]
  plot_data$m[i] = fit_functions[[region]](day)
}
plot_data =
plot_data = plot_data %>%
  group_by(shapeName) %>%
  mutate(map = map / mean(map)) %>%
  pivot_longer(c(map, m)) %>%
