...
load_shangqiu_towns = function(path = "data/china/Shangqiu City Malaria 1980-2023.xlsx",
                               dir = NULL) {
  if (!is.null(dir)) {
...
load_shangqiu = function(path = "data/china/Shangqiu City Malaria 1980-2023.xlsx",
                         dir = NULL) {
  
  data = load_shangqiu_towns(path, dir) %>%
    group_by(Date) %>%
    summarise(Cases = sum(Cases)) %>%
    mutate(logCases = log(Cases + 0.01))
